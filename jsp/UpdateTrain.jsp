...
<body>
	...
	<div class="page-content p-5" id="content">
		...
		String trianNumber = request.getParameter("TrainNumber");
		Train train = dao.getRecordByTrainNumber(trianNumber);
		%>
			...
			<div class="col-lg-7">
				...
				<div class="form-style-2">
					<div class="cantainer">
						...
						<form action="TrainServlet" method="post">
							<label for="field1"><span>Train Name:</span> 
							<input type="text" class="input-field" name="trainName" value="<%=train.getTrainName()%>" readonly="readonly" /> 
							</label> 
							
							<label for="field2"><span>Train Number:</span> 
								<input type="text" class="input-field" name="trainNumber" value="<%=train.getTrainNumber()%>" readonly="readonly" />
							</label>
								 
							<label for="field2"><span>Train Type:</span> 
								  <input type="text" class="input-field" name="trainTypee" value="<%=train.getTrainType()%>" readonly="readonly" /> 
							</label> 
								  
							 <label for="field3"><span>From Station:</span> 
								  <input type="text" class="input-field" name="fromStation" value="<%=train.getFromStation()%>" /> 
							</label> 
								  
							<label for="field4"><span>To Station:</span> 
								  <input type="text" class="input-field" name="toStation" value="<%=train.getToStation()%>" />
							</label>
								   
							<label for="field5"><span>Departure date:</span> 
								    <input type="date" class="input-field" name="departure" value="<%=train.getDeparture()%>" />
							</label> 
								     
							<label for="field5"><span>Departure Time:</span> 
								     <input type="time" class="input-field" name="departureTime" value="<%=train.getDepartureTime()%>" />
							</label>
								      
							<label for="field5"><span>Arrival Time:</span> 
								       <input type="time" class="input-field" name="ArrivalTime" value="<%=train.getArrivalTime()%>" /> 
							</label>
								       
							 <label for="field5"><span>Price(In INR)</span>
								         <input type="number" class="input-field" name="priceINR" value="<%=train.getPrice()%>" /> 
							 </label>
				...
				<script type="text/javascript">
					...
					if (status == "success") {
						...
						swal("Update", "Please Be Carefull While Updating",
								"warning");
					}
				</script>
